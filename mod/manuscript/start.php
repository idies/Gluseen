...
elgg_register_event_handler('init', 'system', 'manuscript_init');
...
function manuscript_init() {
	...
	elgg_register_page_handler('manuscript', 'manuscript_page_handler');

	// Add a new file widget
	elgg_register_widget_type('filerepo', elgg_echo("file"), elgg_echo("file:widget:description"));

	// Register URL handlers for files
	elgg_register_entity_url_handler('object', 'manuscript', 'manuscript_url_override');
	elgg_register_plugin_hook_handler('entity:icon:url', 'object', 'manuscript_icon_url_override');

	// Register granular notification for this object type
	register_notification_object('object', 'manuscript', elgg_echo('file:newupload'));

	// Listen to notification events and supply a more useful message
	elgg_register_plugin_hook_handler('notify:entity:message', 'object', 'manuscript_notify_message');
	...
	elgg_register_plugin_hook_handler('register', 'menu:owner_block', 'manuscript_owner_block_menu');
 ...
 * @return bool
 */
function manuscript_page_handler($page) {
 ...
 */
function manuscript_register_toggle() {
 ...
 * @param array  $params
 */
function manuscript_notify_message($hook, $entity_type, $returnvalue, $params) {
 ...
 */
function manuscript_owner_block_menu($hook, $type, $return, $params) {
	if (elgg_instanceof($params['entity'], 'user')) {
		$url = "manuscript/owner/{$params['entity']->username}";
 ...
 */
function manuscript_get_simple_type($mimetype) {
...
function get_general_manuscript_type($mimetype) {
	elgg_deprecated_notice('Use file_get_simple_type() instead of get_general_manuscript_type()', 1.8);
 ...
 */
function manuscript_get_type_cloud($container_guid = "", $friends = false) {
...
function get_manuscripttype_cloud($owner_guid = "", $friends = false) {
	elgg_deprecated_notice('Use manuscript_get_type_cloud instead of get_manuscripttype_cloud', 1.8);
	return manuscript_get_type_cloud($owner_guid, $friends);
}
 ...
 */
function manuscript_url_override($entity) {
	$title = $entity->title;
 ...
 */
function manuscript_icon_url_override($hook, $type, $returnvalue, $params) {
	$file = $params['entity'];
	$size = $params['size'];
