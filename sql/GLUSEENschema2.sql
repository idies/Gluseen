...
--========================================================
USE master
IF EXISTS(SELECT * FROM sys.databases WHERE name='GLUSEEN')
DROP DATABASE GLUSEEN

CREATE DATABASE GLUSEEN
...
alter table Habitat add constraint PK_Habitat primary key clustered (HabitatID);
...
alter table Load add constraint pkey_Load primary key clustered (pkey)
...
alter table Plot add constraint FK_Plot_Habitat foreign key (HabitatID) references Habitat(HabitatID);
...
alter table DecompSample add constraint FK_DecompSample_Load  foreign key(pkeyLoad) references Load(pkey)
